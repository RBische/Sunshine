...
public class TodayWidgetIntentService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        for (int appWidgetId : appWidgetIds) {
            ...
            if (widgetWidth >= largeWidth) {
                ...
                layoutId = R.layout.widget_normal;
            } else {
                layoutId = R.layout.widget_mini;
            }
            RemoteViews views = new RemoteViews(getPackageName(), layoutId);

            // Add the data to the RemoteViews
            views.setImageViewResource(R.id.ivWeather, weatherArtResourceId);
            // Content Descriptions for RemoteViews were only added in ICS MR1
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH_MR1) {
                setRemoteContentDescription(views, description);
            }
            views.setTextViewText(R.id.tvDescription, description);
            views.setTextViewText(R.id.tvTemperature, formattedMaxTemperature);
            views.setTextViewText(R.id.tvTemperatureMin, formattedMinTemperature);
    ...
    private void setRemoteContentDescription(RemoteViews views, String description) {
        views.setContentDescription(R.id.ivWeather, description);
    }
}