 ...
 */
package fr.bischof.raphael.sunshine.data;
...
public class WeatherProvider extends ContentProvider {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case WEATHER: {
                nbRows = db.delete(WeatherContract.WeatherEntry.TABLE_NAME, selection, selectionArgs);
                break;
            }
            case LOCATION: {
                nbRows = db.delete(WeatherContract.LocationEntry.TABLE_NAME, selection, selectionArgs);
                break;
            }
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
        }
        if (nbRows>0||selectionArgs==null){
            getContext().getContentResolver().notifyChange(uri, null);
        }
    ...
    public int update(
        ...
        switch (match) {
            case WEATHER: {
                normalizeDate(values);
                nbRows = db.update(WeatherContract.WeatherEntry.TABLE_NAME, values, selection, selectionArgs);
                break;
            }
            case LOCATION: {
                normalizeDate(values);
                nbRows = db.update(WeatherContract.LocationEntry.TABLE_NAME, values, selection, selectionArgs);
                break;
            }
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
    ...
    public Cursor getWeather(Uri uri, String[] projection, String sortOrder) {
        final SQLiteDatabase db = mOpenHelper.getReadableDatabase();
